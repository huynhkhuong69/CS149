/**
 * Description: Develop a proc_manager program that executes multiple commands. 
  It reads a command for each execution from stdin on one line of input. 
  So, it reads in the command and its parameters to execute from the stdin (FD0 - file descriptor 0) with each line specifying one command execution. 
  execute one command and its parameters as specified on each line of stdin input. 
  The proc_manager program will start up all the processes with the exec command and wait for them to complete, while each command will write its stdout and stderr to log files.
 * Author names: Khuong Huynh
 * Author emails: khuong.huynh@sjsu.edu
 * Creation date: 03/17/23
 **/
...
void execute_command(char *command, int cmd_index) {
    ...
    if (pid == -1) {
        ...
        //Check for the child of the parent
        //If everything success print out message
        //Else kill signal
        if (WIFEXITED(status)) {
