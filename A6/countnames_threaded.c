/**
 * Description: You will modify your assignment 2 countnames_parallel code to employ multi-threading with two threads  (thread1 is to be assigned the first file and thread2 the second file passed as arguments......),
 * thus creating a new program countnames_threaded. The functionality is the same as A2, but we will simplify things by assuming you expect exactly two input files of names.
 * Your assignment 2 implementation may be a good place to start, but you want to remove the multi-processing parts (fork, wait, etc).
 * If the user gives one or three or four files your program will print a suitable error message and exit.
 * Thus, you will have two threads that update the same counts of names in parallel.
 * Author names: Khuong Huynh
 * Author emails: khuong.huynh@sjsu.edu
 * Creation date: 04/24/23
 **/
